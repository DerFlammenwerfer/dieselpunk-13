/datum/gear/factionlonestar/cloakguild
	display_name = "cloak, lonestar"
	path = /obj/item/clothing/suit/hooded/cloak/job/guild
	allowed_roles = list("Resource Officer","Cargo Technician","Shipping Manager","Shipping Manager","Lonestar Miner","Bartender","Chef","Gardener","Janitor")
	slot = slot_wear_suit
	sort_category = "Faction: Lonestar"

/datum/gear/factionlonestar/cloakguildminer
	display_name = "cloak, lonestar miner"
	path = /obj/item/clothing/suit/hooded/cloak/job/mining
	allowed_roles = list("Resource Officer","Cargo Technician","Shipping Manager","Lonestar Miner","Bartender","Chef","Gardener","Janitor")
	slot = slot_wear_suit
	sort_category = "Faction: Lonestar"

/datum/gear/factionlonestar/cloakgm
	display_name = "cloak, CEO"
	path = /obj/item/clothing/suit/hooded/cloak/job/gm
	allowed_roles = list("Resource Officer")
	slot = slot_wear_suit
	sort_category = "Faction: Lonestar"

/datum/gear/factionlonestar/snowsuitguild
	display_name = "snowsuit, lonestar"
	path = /obj/item/clothing/suit/storage/snowsuit/cargo
	allowed_roles = list("Resource Officer","Cargo Technician","Shipping Manager","Lonestar Miner","Bartender","Chef","Gardener","Janitor")
	slot = slot_wear_suit
	sort_category = "Faction: Lonestar"

/datum/gear/factionlonestar/winterbootsguild
	display_name = "winter boots, lonestar"
	path = /obj/item/clothing/shoes/winter/supply
	allowed_roles = list("Resource Officer","Cargo Technician","Shipping Manager","Lonestar Miner","Bartender","Chef","Gardener","Janitor")
	slot = slot_shoes
	sort_category = "Faction: Lonestar"

/datum/gear/factionlonestar/winterbootsguildminer
	display_name = "winter boots, lonestar miner"
	path = /obj/item/clothing/shoes/winter/mining
	allowed_roles = list("Lonestar Miner")
	slot = slot_shoes
	sort_category = "Faction: Lonestar"

/datum/gear/factionlonestar/winterguild
	display_name = "winter coat, lonestar"
	path = /obj/item/clothing/suit/hooded/wintercoat/cargo
	allowed_roles = list("Resource Officer","Cargo Technician","Shipping Manager","Lonestar Miner","Bartender","Chef","Gardener","Janitor")
	slot = slot_wear_suit
	sort_category = "Faction: Lonestar"

/datum/gear/factionlonestar
	display_name = "winter coat, lonestar miner"
	path = /obj/item/clothing/suit/hooded/wintercoat/cargo/miner
	allowed_roles = list("Lonestar Miner")
	slot = slot_wear_suit
	sort_category = "Faction: Lonestar"
