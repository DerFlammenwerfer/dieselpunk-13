...
/obj/item/weapon/implant/core_implant/soulcrypt/proc/send_death_message() //Sends the death message whenever the person who has this dies.
	...
	switch(was_emp)
		if(TRUE)
			to_chat(host_mind.current, SPAN_WARNING("Your soulcrypt has been subjected to an electromagnetic pulse, and thus your neural engrams are unreliable. It's automatically pruning the unsalvagable memories, about an hour's worth. Perhaps more."))
		if(FALSE)
...
/obj/item/weapon/implant/core_implant/soulcrypt/proc/send_revive_notice() //Triggered by implantation into a mindless mob.
	...
	switch(was_emp)
		if(TRUE)
			to_chat(host_mind.current, SPAN_WARNING("Your soulcrypt has been subjected to an electromagnetic pulse, and you're missing about an hour of memory from before your death."))
		if(FALSE)
